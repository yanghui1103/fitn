	...
	function onClick(event, treeId, treeNode, clickFlag) { 
		...
		ajaxTodo(ctx+url,function(data){
			if(data.hasMenu=="1"){ // 角色有这个菜单的话
				var obj = $('<label><input type=checkbox name=temp_arr1 value='+treeNode.id+' checked=checked />'+treeNode.name+'</label>');
				$("#menuListR",navTab.getCurrentPanel()).append(obj); 
			}else{
				var obj = $('<label><input type=checkbox name=temp_arr1 value='+treeNode.id+'  />'+treeNode.name+'</label>');
				$("#menuListR",navTab.getCurrentPanel()).append(obj); 
			}
			if(data.res=="2"){
				...
				for(var i=0;i<list_operation.length;i++){ 
					if("1"==list_operation[i].checked){
						var obj = $('<label><input type=checkbox name=temp_arr2 value='+list_operation[i].id+' checked=checked />'+list_operation[i].name+'</label>');
						$("#operationListR",navTab.getCurrentPanel()).append(obj); 
					}else{
						var obj = $('<label><input type=checkbox name=temp_arr2 value='+list_operation[i].id+'  />'+list_operation[i].name+'</label>');
						$("#operationListR",navTab.getCurrentPanel()).append(obj); 
					}
				} // 功能结束
				...
				for(var i=0;i<list_elements.length;i++){
					if("1"==list_elements[i].checked){
						var obj = $('<label><input type=checkbox name=temp_arr3 value='+list_elements[i].id+' checked=checked />'+list_elements[i].name+'</label>');
						$("#elementListR",navTab.getCurrentPanel()).append(obj); 
					}else{
						var obj = $('<label><input type=checkbox name=temp_arr3 value='+list_elements[i].id+'  />'+list_elements[i].name+'</label>');
						$("#elementListR",navTab.getCurrentPanel()).append(obj); 
					}
				} //页面元素
				...
				for(var i=0;i<list_att.length;i++){
					if("1"==list_att[i].checked){
						var obj = $('<label><input type=checkbox name=temp_arr4 value='+list_att[i].id+' checked=checked />'+list_att[i].name+'</label>');
						$("#attListR",navTab.getCurrentPanel()).append(obj); 
					}else{
						var obj = $('<label><input type=checkbox name=temp_arr4 value='+list_att[i].id+'  />'+list_att[i].name+'</label>');
						$("#attListR",navTab.getCurrentPanel()).append(obj); 
					}
				} //附件
			}
